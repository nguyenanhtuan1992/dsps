<!-- Start Menu panel -->
<div class="navbar navbar-default navbar-horizontal">
    <div class="navbar-header"><a class="navbar-brand" href="#">Da Nang</a></div>
    <div class="navbar-menu">
        <ul class="nav navbar-nav">
            <li class="dropdown"><a href="javascript:void(0);" tabindex="0" data-toggle="dropdown"
                                    data-submenu>Event</a>
                <ul class="dropdown-menu">
                    <li><a href="#" tabindex="0">Super Event
                        management</a></li>
                    <li><a href="#" tabindex="0">Delegation
                        Management</a></li>
                    <li><a href="#" tabindex="0">Officials
                        management</a></li>
                    <li><a href="#" tabindex="0">Sub Event Management</a>
                    </li>
                </ul>
            </li>
            <li class="dropdown"><a href="javascript:void(0);" tabindex="0" data-toggle="dropdown"
                                    data-submenu>Police</a>
                <ul class="dropdown-menu">
                    <li><a href="#" tabindex="0">Police Management</a></li>
                    <li><a href="#" tabindex="0">Vehicle Management</a>
                    </li>
                </ul>
            </li>
            <li class="dropdown"><a href="javascript:void(0);" tabindex="0" data-toggle="dropdown"
                                    data-submenu>Place</a>
                <ul class="dropdown-menu">
                    <li><a href="#" tabindex="0">Place Management</a></li>
                </ul>
            </li>
            <li class="dropdown"><a href="javascript:void(0);" tabindex="0" data-toggle="dropdown"
                                    data-submenu>Other</a>
                <ul class="dropdown-menu">
                    <li><a href="#" tabindex="0">Position Management</a>
                    </li>
                    <li><a href="#" tabindex="0">Nation Management</a></li>
                    <li><a href="#" tabindex="0">Police Type
                        Management</a></li>
                    <li><a href="#" tabindex="0">Organization
                        management</a></li>
                    <li><a href="#" tabindex="0">Badge Management</a></li>
                    <li><a href="#" tabindex="0">Street Management</a></li>
                    <li><a href="#" tabindex="0">Vehicle Type
                        Management</a></li>
                </ul>
            </li>
            <li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button"
                                    aria-expanded="false">Account</a>
                <ul class="dropdown-menu" role="menu">
                    <li><a href="#">Thong Tin Tai Khoan</a></li>
                    <li class="divider"></li>
                    <li><a href="#">Thay Doi Mat Khau</a></li>
                </ul>
            </li>
            <li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button"
                                    aria-expanded="false">Administrator</a>
                <ul class="dropdown-menu" role="menu">
                    <li><a href="#">Account rule definition</a></li>
                    <li class="divider"></li>
                    <li><a href="#">Account management</a></li>
                    <li class="divider"></li>
                    <li><a href="#">Role management</a></li>
                    <li class="divider"></li>
                    <li><a href="#">Message management</a></li>
                </ul>
            </li>
        </ul>
        <ul class="nav navbar-nav navbar-side" style="float: right; padding-right: 50px;">
            <li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button"
                                    aria-expanded="false" style="font-size: 20px;">APEC</a>
                     <ul class="dropdown-menu dropdown-nav-left" role="menu">
                            <li><a href="${pageContext.request.contextPath}/jsp/index.jsp">&nbsp;&nbsp;Thay Doi</a></li>
                     </ul>
            </li>
             <li><a href="login.html"><span class="glyphicon glyphicon-log-out"></span>&nbsp;&nbsp;Logout</a></li>
        </ul>
    </div>
</div>